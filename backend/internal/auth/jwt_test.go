...
func TestGenerateToken_ReturnsValidToken(t *testing.T) {
	...
	token, expiresAt, err := GenerateToken(userID, secret, expiry)
...
func TestValidateToken_ValidToken(t *testing.T) {
	...
	token, _, err := GenerateToken(userID, secret, expiry)
	require.NoError(t, err)

	parsedID, err := ValidateToken(token, secret)
	require.NoError(t, err)
...
func TestValidateToken_WrongSecret(t *testing.T) {
	userID := uuid.New()
	token, _, err := GenerateToken(userID, "secret-1", 1*time.Hour)
	require.NoError(t, err)
...
func TestValidateToken_ExpiredToken(t *testing.T) {
	...
	token, _, err := GenerateToken(userID, secret, -1*time.Hour)
	require.NoError(t, err)
...
func TestGenerateToken_ContainsCorrectUserID(t *testing.T) {
	...
	recovered, err := ValidateToken(token, secret)
	require.NoError(t, err)

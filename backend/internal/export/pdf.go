...
func GeneratePDF(data *PDFReportData, cfg *config.Config, outputDir string) (string, error) {
	...
	pdf.Ln(5)
	setFont("B", 14)
	gradeLabel := getGradeLabel(grade, true)
	...
	indicators := getIndicatorRows(data.Assessment, true)
	...
	if len(data.Issues) > 0 {
		...
		for i, issue := range data.Issues {
			if i >= 20 {
				break // limit to 20 issues for readability
			}
			fill := i%2 == 0
			pdf.SetFillColor(245, 245, 245)
			pdf.CellFormat(probWidths[0], 7, issue.Severity, "1", 0, "C", fill, 0, "")
			desc := truncateString(issue.Description, 80)
			pdf.CellFormat(probWidths[1], 7, desc, "1", 0, "L", fill, 0, "")
			pdf.Ln(-1)
		}
	} else {
		setFont("", 10)
	...
	for i, rule := range data.Session.RulesApplied {
		ruleLabel := getRuleLabel(rule, true)
		pdf.CellFormat(190, 7, fmt.Sprintf("%d. %s", i+1, ruleLabel), "", 1, "L", false, 0, "")
	}

	if len(data.Session.RulesApplied) == 0 {
		noRules := "\xe6\x9c\xaa\xe5\xa5\x97\xe7\x94\xa8\xe4\xbb\xbb\xe4\xbd\x95\xe8\xa6\x8f\xe5\x89\x87"
		pdf.CellFormat(190, 7, noRules, "", 1, "L", false, 0, "")
	}

	// Save PDF
	filename := fmt.Sprintf("report_%s.pdf", data.Session.ID.String()[:8])

...
export async function registerUser(page: Page, username: string, password: string): Promise<void> {
  ...
  await page.locator('input[placeholder*="使用者名稱"], input[placeholder*="account"], input[placeholder*="Account"]').first().fill(username)
  await page.locator('input[type="password"]').first().fill(password)
  await page.locator('input[placeholder*="再次"], input[placeholder*="Re-enter"], input[placeholder*="Confirm"]').fill(password)
...
export async function ensureTestUser(page: Page): Promise<void> {
  ...
  await page.locator('input[placeholder*="使用者名稱"], input[placeholder*="account"], input[placeholder*="Account"]').first().fill(TEST_USER.username)
  await page.locator('input[type="password"]').first().fill(TEST_USER.password)
  await page.locator('input[placeholder*="再次"], input[placeholder*="Re-enter"], input[placeholder*="Confirm"]').fill(TEST_USER.password)
  await page.getByRole('button', { name: /註冊|register/i }).click()
...
export async function ensureAdminUser(page: Page): Promise<void> {
  await page.goto('./register')
  await page.waitForTimeout(500)
  await page.locator('input[placeholder*="使用者名稱"], input[placeholder*="account"], input[placeholder*="Account"]').first().fill(ADMIN_USER.username)
  await page.locator('input[type="password"]').first().fill(ADMIN_USER.password)
  await page.locator('input[placeholder*="再次"], input[placeholder*="Re-enter"], input[placeholder*="Confirm"]').fill(ADMIN_USER.password)
  await page.getByRole('button', { name: /註冊|register/i }).click()
